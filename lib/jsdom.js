...
exports.jsdom = function (html, options) {
  ...
  window.document.defaultView.localStorage = new StorageMock();
  window.document.defaultView.sessionStorage = new StorageMock();
  window.document.defaultView.matchMedia = function (media) {
    return {
      matches: true,
      media: media || "all",
...
function processHTML(config) {
  ...
  if (config.scripts.length > 0 || config.src.length > 0) {
    ...
    setTimeout(scriptComplete, 120000);
